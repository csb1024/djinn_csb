...
ITERS=(128 64 32 16 8)
#ITERS=(10)
iter_num=${#ITERS[@]}

#FLAGS
mon_flag=1 # flag for running monitor
flag_1=1 # CPU total life span experiment 
flag_2=0 # GPU total life span & utilization experiment
flag_3=0 
...
batch=${BATCHES[${i}]}
iters=${ITERS[${i}]}
RESULT_DIR=$CLIENT_DIR/results/$repeat-$batch
mkdir -p $RESULT_DIR
...
# 3 the real experimnet
	...
	do
		...
		then
			cd $CLIENT_DIR/nlp
			$NLP_SH $app $iters $batch 0 &
			sleep 10
		else
			cd $CLIENT_DIR/img
			$IMG_SH $app $iters $batch 0 &
			sleep 20
		fi
		pids[${h}]=$!
		# wait for all pids
