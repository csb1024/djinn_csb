...
def callPos(batch_size,sync):
	p=subprocess.Popen(['./tonic-nlp','--task','pos','--network','pos.prototxt','--weight', 'pos.caffemodel','--input','input/sanity-test-input.txt', '--djinn', '0',"--gpu","1",'--portno', PORT,'--batch',str(batch_size)],shell=False,cwd=os.getcwd()+"/nlp")
	if sync:
		p.wait()
		return
	return p
...
def callNer(batch_size,sync):
	p=subprocess.Popen(['./tonic-nlp','--task','ner','--network','ner.prototxt','--weight', 'ner.caffemodel','--input','input/sanity-test-input.txt', '--djinn', '0',"--gpu","1",'--portno', PORT,'--batch',str(batch_size)],shell=False,cwd=os.getcwd()+"/nlp")
	if sync: 
		p.wait()
		return
	return
...
def callChk(batch_size,sync):
	p=subprocess.Popen(['./tonic-nlp','--task','chk','--network','chk.prototxt','--weight', 'chk.caffemodel','--input','input/sanity-test-input.txt', '--djinn', '0',"--gpu","1",'--portno', PORT,'--batch',str(batch_size)],shell=False,cwd=os.getcwd()+"/nlp")
	if sync:
		p.wait()
		return
	return p
...
#  	large_batch_vec = [1,2,4,8,16,32,64,128,256,512,1024]
  	large_batch_vec = [1,2,4,8,16,32,64,128,256]
